...
type RoleType = RelationshipRoleTemplateType;
...
export interface PersonalityTraits {
  ...
  nurturing: number; // 0-1: independent to nurturing
  authority: number; // 0-1: peer to authority figure
}
...
export class AIPersonalityService {
  private rolePersonalities: Map<RoleType, RolePersonality>;
   ...
   */
  getPersonality(roleType: RoleType): RolePersonality | undefined {
   ...
   */
  getAllPersonalities(): RolePersonality[] {
  ...
  selectResponsePattern(
    roleType: RoleType,
    emotionalContext: 'greeting' | 'comfort' | 'advice' | 'celebration' | 'concern' | 'curiosity'
  ): string[] {
   ...
   */
  getConversationStyle(roleType: RoleType): ConversationStyle | undefined {
    const personality = this.getPersonality(roleType);
   ...
   */
  isTopicAppropriate(roleType: RoleType, topic: string): boolean {
    const personality = this.getPersonality(roleType);
    if (!personality) return false;

    const isAvoided = personality.avoidTopics.some(
      (avoidTopic) => topic.toLowerCase().includes(avoidTopic.toLowerCase())
    );
   ...
   */
  getTraitsDescription(roleType: RoleType): string {
    const personality = this.getPersonality(roleType);
    if (!personality) return '';
...
- Authority: ${this.traitToText(traits.authority, 'peer', 'authority')}
  ...
  private traitToText(value: number, lowLabel: string, highLabel: string): string {
