...
exports.conversationMemoryService = exports.ConversationMemoryService = void 0;
...
class ConversationMemoryService {
     ...
     */
    async getConversationContext(relationshipId, maxRecentMessages = 20) {
        ...
        const relationshipDuration = Math.floor((Date.now() - relationship.createdAt.getTime()) / (1000 * 60 * 60 * 24));
        // Get total message count
        const totalMessages = await prisma_1.prisma.conversationMessage.count({
            where: { relationshipId },
        });
        // Format recent messages
        const recentMessages = relationship.messages
            .reverse()
            .map((msg) => ({
            id: msg.id,
            content: msg.content,
        ...
        const userPreferences = relationship.user.preferences || {};
     ...
     */
    async getImportantMoments(relationshipId, limit = 10) {
     ...
     */
    async extractThemes(relationshipId) {
        // Get recent messages
        const messages = await prisma_1.prisma.conversationMessage.findMany({
            where: { relationshipId },
            orderBy: { createdAt: 'desc' },
        ...
        for (const message of messages) {
            ...
            if (metadata?.themes) {
                metadata.themes.forEach((theme) => themes.add(theme));
            }
     ...
     */
    async generateMemorySummary(relationshipId) {
     ...
     */
    async buildContextPrompt(relationshipId) {
        ...
        if (context.lastInteractionDate) {
            const daysSinceLastInteraction = Math.floor((Date.now() - context.lastInteractionDate.getTime()) / (1000 * 60 * 60 * 24));
     ...
     */
    async storeMessage(relationshipId, senderId, content, emotionalTone, metadata) {
        const message = await prisma_1.prisma.conversationMessage.create({
            data: {
                relationshipId,
                senderId,
                content,
    ...
    async markAsImportant(messageId) {
        await prisma_1.prisma.conversationMessage.update({
            where: { id: messageId },
            data: {
                metadata: {
                    important: true,
     ...
     */
    async searchMessages(relationshipId, query, limit = 10) {
        const messages = await prisma_1.prisma.conversationMessage.findMany({
            where: {
                relationshipId,
                content: {
    ...
    async identifyMilestones(relationshipId, context) {
        const milestones = [];
