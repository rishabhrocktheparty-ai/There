...
exports.socialAuthRouter = void 0;
...
function generateToken(userId, email, isAdmin = false) {
    return jsonwebtoken_1.default.sign({ id: userId, email, isAdmin }, config_1.config.JWT_SECRET, { expiresIn: '7d' });
}
/**
...
router.get('/:provider/authorize', async (req, res, next) => {
    try {
        ...
        if (!['google', 'apple', 'github'].includes(provider)) {
            throw new errorHandler_1.HttpError(400, 'Unsupported provider');
        }
        if (!redirect_uri || typeof redirect_uri !== 'string') {
            throw new errorHandler_1.HttpError(400, 'redirect_uri is required');
        }
        ...
        logger_1.logger.info(`OAuth flow initiated: ${provider} - state: ${state}`);
 ...
 */
router.get('/:provider/mock-consent', async (req, res, next) => {
    try {
        const { provider } = req.params;
        const { state, redirect_uri } = req.query;
        if (process.env.NODE_ENV !== 'development') {
            throw new errorHandler_1.HttpError(404, 'Not found');
        }
        if (!state || typeof state !== 'string') {
            throw new errorHandler_1.HttpError(400, 'Invalid state');
        }
        // Verify state exists
        const session = oauthSessions.get(state);
        if (!session) {
            throw new errorHandler_1.HttpError(400, 'Invalid or expired state');
        }
        // Get mock profiles for this provider
        const profiles = MOCK_PROFILES[provider] || [];
...
router.get('/:provider/callback', async (req, res, next) => {
    try {
        ...
        if (error) {
            logger_1.logger.warn(`OAuth error: ${provider} - ${error}`);
            return res.redirect(`${process.env.CORS_ORIGIN}?error=${error}`);
        }
        if (!code || typeof code !== 'string') {
            throw new errorHandler_1.HttpError(400, 'Authorization code required');
        }
        if (!state || typeof state !== 'string') {
            throw new errorHandler_1.HttpError(400, 'State required');
        }
        // Verify state
        const session = oauthSessions.get(state);
        if (!session) {
        ...
        if (session.provider !== provider) {
            throw new errorHandler_1.HttpError(400, 'Provider mismatch');
        }
        // Remove used session
        ...
        if (process.env.NODE_ENV === 'development' && code.startsWith('mock_code_')) {
            ...
            const mockData = req.sessionStorage?.getItem(`mock_oauth_${code}`);
            if (mockData) {
        ...
        let user = await prisma_1.prisma.user.findFirst({
            where: {
                OR: [
                    { externalId, authProvider: provider.toUpperCase() },
                    { email },
                ],
            },
        ...
        if (!user) {
            ...
            logger_1.logger.info(`New user created via ${provider}: ${email}`);
        }
        else {
            ...
            logger_1.logger.info(`Existing user logged in via ${provider}: ${email}`);
        }
        // Generate JWT token
 ...
 */
router.post('/:provider/token', async (req, res, next) => {
    try {
        const { provider } = req.params;
        const { accessToken, idToken } = req.body;
        if (!accessToken && !idToken) {
            throw new errorHandler_1.HttpError(400, 'Access token or ID token required');
        }
        ...
        if (!user) {
            ...
            logger_1.logger.info(`New user created via ${provider} token: ${email}`);
        }
        // Generate JWT token
        const token = generateToken(user.id, user.email, !!user.adminProfile);
        res.json({
 ...
 */
router.post('/user/social-login', async (req, res, next) => {
    try {
        const { provider, accessToken } = req.body;
        if (!provider || !accessToken) {
            throw new errorHandler_1.HttpError(400, 'Provider and accessToken required');
        }
        // In development, accept mock tokens
        let profileData;
        if (process.env.NODE_ENV === 'development' && accessToken.startsWith('fake-')) {
            ...
            if (!profiles || profiles.length === 0) {
                throw new errorHandler_1.HttpError(400, `Unsupported provider: ${provider}`);
            }
            profileData = profiles[0];
        }
        else {
        ...
        if (!user) {
            ...
            logger_1.logger.info(`New user created via ${provider} (simplified endpoint): ${email}`);
        }
        // Generate JWT token
        const token = generateToken(user.id, user.email, !!user.adminProfile);
        res.json({
 ...
 */
async function verifyTokenAndFetchProfile(provider, token) {
...
exports.socialAuthRouter = router;
